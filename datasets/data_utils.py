...
def random_crop_m(img, gt, gt2, roi, size=[0.2, 0.8]):
    """Crop patches in ROI with random size"""
    import random
...
def soft_centroid(a: Tensor) -> Tensor:
    ...
    ws, hs = map_(
        lambda e: Tensor(e).to(a.device).type(torch.float32), np.mgrid[0:w, 0:h]
    )
...
def remove_hair(img):
    ...
    blackhat = cv2.morphologyEx(grayScale, cv2.MORPH_BLACKHAT, kernel=kernel)
